...
public class JsonConfig {
    ...
    public JsonConfig(String name, File file) throws IOException {
        this.name = name;
        this.file = file;
        ...
        if(jsonElement instanceof JsonNull) jsonObject = new JsonObject();
        else jsonObject = jsonElement.getAsJsonObject();
    }

    public boolean contains(String key) {
        if(!key.contains(".")) jsonObject.has(key);

        String[] path = key.split("\\.");
        JsonElement currentElement = jsonObject;

        for (String subPath : path) {
            if(!currentElement.isJsonObject()) return false;
            if(!currentElement.getAsJsonObject().has(subPath)) return false;
    ...
    public JsonElement get(String key) {
        if(!contains(key)) return JsonNull.INSTANCE;
        if(!key.contains(".")) jsonObject.get(key);

        String[] path = key.split("\\.");
        JsonElement currentElement = jsonObject;

        for (String subPath : path) {
            if(!currentElement.isJsonObject()) return JsonNull.INSTANCE;
            if(!currentElement.getAsJsonObject().has(subPath)) return JsonNull.INSTANCE;
